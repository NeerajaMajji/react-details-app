...
const StoreApp=()=>{
    ...
    const handleCategoryName=(event)=>{
       const appres= event.target.value
       setCategoryName(appres)
        const handeltabs=appsList.filter((item)=>item.category===appres.toUpperCase())
         setFilterApp(handeltabs);
    }
   
    return(
        <div>
            {CategoryList.map((each)=>(<button key={each.storename} value={each.storename} onClick={handleCategoryName}>{each.storename}</button>))}
            {categoryName && (
            <div>
               { filterApp.map((each)=>(<img key={each.appId} src={each.imageUrl} alt={each.appName}/>))}
            </div>
            )
            }
        </div>
